...
/*   Updated: 2024/05/26 20:37:10 by passunca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_meal(t_philo *philo)
{
	...
	usleep(ft_min(philo->t_meal, philo->t_death));
...
int	ft_think(t_philo *philo)
{
	if (ft_log(philo, THINK, ft_now(philo)) == 1)
		return (1);
	usleep(philo->t_think);
	return (0);
}

int	ft_log(t_philo *p, int select, t_time t)
{
	if (sem_wait(p->sem_printf) == 0)
	{
		if (select == FORK)
			printf("%lld %d has taken a fork\n", \
					ft_dtime(p->t_start, t), p->idx);
		else if (select == EAT)
